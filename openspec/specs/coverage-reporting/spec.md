...
The **coverage-reporting** capability measures which source lines each test layer
executes, and publishes the layers separately so the combined figure describes the whole
of what is tested rather than the half one runner can see.

Saldo's two layers cover near-complementary halves of the codebase: Jest holds the pure
and server layers, while the Playwright suite drives every page and component. Measuring
only the first reports a number that reads as "half the app is untested" when the truth
is "half the app is tested by a layer the report cannot see".

This capability is about execution, not assertion. A file is covered here because it ran,
not because anything checked what it did — `spec-test-traceability` is what answers that,
and it answers it better. The job here is reachability: which code no layer executes at
all.
