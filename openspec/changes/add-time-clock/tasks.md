...
- [ ] 1.1 Add `started_at DateTime?` to the `User` model in `prisma/schema.prisma`.
- [ ] 1.2 Create the migration (hand-written if needed) and run it; `prisma generate`.
- [ ] 1.3 Add an active-session type to `src/types` (e.g. `ActiveSession { startedAt: Date }` or `null`).

## 2. Repository + actions

- [ ] 2.1 Add a clock repository module: `getActiveSession()` (reads `started_at` for the session user), `clockIn(startedAt)` (sets it; no-op if already set), `clearSession()` (nulls it). Wrap DB calls in `Sentry.startSpan`; gate on `getUserFromSession()`.
- [ ] 2.2 Add actions in `src/actions`: `onClockIn(startedAt)`, `onClockOut(data)` (validate, create worklog via `insertWorklog`, then clear session), `onClockDiscard()` (clear session, no worklog). Validate the worklog data with the existing `WorklogSchema` (enforces same-day + to>from).
- [ ] 2.3 Ensure clock-out save path reuses `insertWorklog` so all existing worklog validation/ownership applies.
